...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        function loadtabeljadwal(data) {
            ...
            $('#formDetailJadwal').attr('style','display: none');
        ...
        function detailJadwal(kodeJadwal) {
            $.post('<?=url('jadwal/detailbykode')?>',{ Kode_jadwal:kodeJadwal},
                function(data) {
                    $('#formDetailJadwal').attr('style','display: block');
                    $('#namaInstansi').text(data.Nama_instansi);
                    $('#namaRuangan').text(data.Nama_kelas);
                    $('#kodeRuangan').text(data.Kode_ruangan);
                    $('#namaSubjek').text(data.Nama_mata_pelajaran);
                    $('#kodeSubjek').text(data.Kode_jadwal);
                    $('#tanggal').text(data.startdate);
                    $('#jam').text(data.starttime+" S/D "+data.endtime);
                    $("#jadwaltabel tr").remove();
                    loadDataAbsen(data.maclist);
                }
            );
        }
        function loadDataAbsen(data) {
            var table = document.getElementById("jadwaltabel");
            var thead, tr, td;
            table.appendChild(thead = document.createElement("thead"));
            thead.appendChild(tr = document.createElement("tr"));
            ...
            var count=1;
            for (var i = 0; i < data.length; i++) {
                var listAbsen=data[i].List_absen;
                for (var j=0;j<listAbsen.length;j++){
                    tr = document.createElement("tr");
                    tr.setAttribute("id", "row" + i);
                    if (i%2 == 0)
                    {
                    ...
                    tr.appendChild(td = document.createElement("td"));
                    td.innerHTML =listAbsen[j].rf_id;
                    tr.appendChild(td = document.createElement("td"));
                    td.innerHTML =listAbsen[j].date;
                    tr.appendChild(td = document.createElement("td"));
                    td.innerHTML =listAbsen[j].time;
                    count++;
                }
...
<div  class="ui grid" style="padding: 5%">
    ...
    <div class="thirteen wide column" >
        <form id="formDetailJadwal" class="ui form" style="display: none">
            <h4 class="ui dividing header">Detail Jadwal</h4>
            <h3 id="namaInstansi">PPTIK</h3>
            <div class="field">
                <div class="three fields">
                    <div class="field">
                        <label>Nama Ruangan</label>
                        <label class="ui green label" id="namaRuangan">Nama Ruangan</label>
                    </div>
                    <div class="field">
                        <label>Kode Ruangan</label>
                        <label class="ui green label" id="kodeRuangan">Kode Ruangan</label>
                    </div>
                </div>
            </div>
            <div class="field">
                <div class="three fields">
                    <div class="field">
                        ...
                        <label class="ui green label" id="namaSubjek">Nama Subjek</label>
                    </div>
                    <div class="field">
                        <label>Kode Subjek </label>
                        <label class="ui green label" id="kodeSubjek">Kode Subjek</label>
                    </div>
                </div>
            </div>
            <div class="field">
                <div class="three fields">
                    <div class="field">
                        <label>Tanggal</label>
                        <label class="ui green label" id="tanggal">Tanggal Jadwal</label>
                    </div>
                </div>
            </div>
            <div class="field">
                <div class="three fields">
                    <div class="field">
                        <label>Jam</label>
                        <label class="ui green label" id="jam">Jam</label>
                    </div>
                </div>
            </div>
        </form>
